...
class CommonTestUtilities : public OpsTestBase {
 public:
  ...
  static void VerifyTensorsGradClose(const GraphGradRunner& run,
    ...
    Tensor grad(dtype, {batch, height, width, depth});
    Tensor input(dtype, {batch, height, width, depth});
    grad.flat<T>() = grad.flat<T>().setRandom();
    test::FillValues<T>(&input, {-13.86, -6.86, -2.51, -1.51, -0.53, 0.00, 0.53,
                                 1.51, 2.51, 6.10, 6.86, 13.86});
...
class ActivationOpsTest : public OpsTestBase {
 protected:
  void VerifyActivationOps(const string& op_name) {
...
TYPED_TEST_P(ActivationOpsTest, ReluOpTest) {
  this->VerifyActivationOps("Relu");
}

TYPED_TEST_P(ActivationOpsTest, Relu6OpTest) {
  this->VerifyActivationOps("Relu6");
}

TYPED_TEST_P(ActivationOpsTest, LeakyReluOpTest) {
  this->VerifyActivationOps("LeakyRelu");
}

TYPED_TEST_P(ActivationOpsTest, TanhOpTest) {
  this->VerifyActivationOps("Tanh");
}

TYPED_TEST_P(ActivationOpsTest, EluOpTest) { this->VerifyActivationOps("Elu"); }
...
class ActivationGradOpsTest : public OpsTestBase {
 protected:
  void VerifyActivationGradOps(const string& op_name) {
