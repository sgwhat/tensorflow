...
#ifndef TENSORFLOW_CORE_COMMON_RUNTIME_PLUGGABLEDEVICE_PLUGGABLEDEVICE_UTIL_H_
#define TENSORFLOW_CORE_COMMON_RUNTIME_PLUGGABLEDEVICE_PLUGGABLEDEVICE_UTIL_H_
...
class PluggableDeviceUtil {
 public:
  // "tensor" is PluggableDevice-local.  "dev" is the hosting Pluggabledevice.
  // "device_context" should be the context of the PluggableDevice "_Send" op
  // which provides the Tensor.
  // Sets all necessary fields of "proto" by transferring value
  // bytes from PluggableDevice to CPU RAM. "is_dead" indicates that the
  // tensor is dead with an uninit value.
  static void SetProtoFromPluggableDevice(const Tensor& tensor, Device* dev,
  ...
  // on "pluggable_device".
  static uint64 Checksum(Device* pluggable_device,
                         const DeviceContext* device_context,
                         const Tensor& tensor);
...
#endif  // TENSORFLOW_CORE_COMMON_RUNTIME_PLUGGABLEDEVICE_PLUGGABLEDEVICE_UTIL_H_
