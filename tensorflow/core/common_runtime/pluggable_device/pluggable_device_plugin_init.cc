...
Status RegisterPluggableDevicePlugin(const char* library_filename) {
  // Step 1: Load plugin
  tensorflow::Env* env = tensorflow::Env::Default();
  void* dso_handle;
  TF_RETURN_IF_ERROR(env->LoadDynamicLibrary(library_filename, &dso_handle));

  // Step2 Init Device Module
  TF_RETURN_IF_ERROR(InitDeviceModule(dso_handle));

  // Step3 Init Kernel Module
