...
// This factory helps to ensure that different Pluggable device objects that
...
PluggableDevice::PluggableDevice(
    ...
    : LocalDevice(options,
                  Device::BuildDeviceAttributes(
                      ...
                      physical_device_desc, subdevice_type.c_str())),
...
Status PluggableDevice::Init(const SessionOptions& options) {
  se::Platform* platform = PluggableDeviceMachineManager(subdevice_type());
  ...
  if (gpu_thread_mode != "global") {
    ...
    if (gpu_thread_mode == "gpu_private") {
      // TODO(zhengxq): since these threads only serve a single PluggableDevice
      // device,
      //   we should set the device context once for each thread, and avoid
      //   setting them for each kernel.
      // TODO(zhengxq): pin the thread to the same socket of the target
      // PluggableDevice.
  ...
  if (attr.on_host()) {
    if (attr.gpu_compatible() || force_gpu_compatible_) {
      PluggableDeviceProcessState* ps =
          PluggableDeviceProcessState::singleton(subdevice_type());
...
string PluggableDevice::ComputeOpKernelDebugString(const OpKernel& op_kernel,
  ...
  return strings::StrCat(op_kernel.name(), " op ", op_kernel.type_string(),
                         " on ", subdevice_type(), tf_device_id_.value(),
                         " stream[", stream_id, "]");
}
...
void PluggableDevice::ComputeAsync(AsyncOpKernel* op_kernel,
  ...
  VLOG(1) << "PluggableDevice::ComputeAsync " << op_kernel->name() << "op"
          << op_kernel->type_string() << " on " << subdevice_type()
