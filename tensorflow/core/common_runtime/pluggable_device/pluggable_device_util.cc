...
void PluggableDeviceUtil::SetProtoFromPluggableDevice(
    const Tensor& tensor, Device* dev, const DeviceContext* device_context,
    TensorProto* proto, bool is_dead, StatusCallback done) {
  VLOG(1) << "SetProtoFromPluggableDevice device_context" << device_context;
  ...
  // Wait for the sender's main stream to make sure that the data is available.
  send_device_to_host_stream->ThenWaitFor(send_stream);

  // Tensor values need to be copied from PluggableDevice to CPU ram so that
  // we can build the protobuf response for a RecvTensor RPC.
  // "device_context" identifiers the stream where the _Send op executed.
  ...
  // over from the PluggableDevice buffer, Not that the 0-size tensors do not
  // have a backing buffer.
  Allocator* alloc = nullptr;
  char* buf = nullptr;
  const int64 total_bytes = is_dead ? 0 : tensor.TotalBytes();
  if (total_bytes > 0) {
    alloc = PluggableDeviceProcessState::singleton(dev->subdevice_type())
    ...
    send_device_to_host_stream->ThenMemcpy(buf, device_src_ptr, total_bytes);
  }
  ...
  dev_info->event_mgr->ThenExecute(
      send_device_to_host_stream, [send_device_to_host_stream, done, proto, buf,
        ...
        if (!send_device_to_host_stream->ok()) {
          LOG(FATAL)
              << "SetProtoFromPluggableDevice: PluggableDevice memcpy failed";
        }
        tensor_ref.Unref();
        if (total_bytes > 0) {
...
void PluggableDeviceUtil::DeviceToDeviceCopy(
    ...
    AllocatorAttributes dst_all_attr, const Tensor* input, Tensor* output,
...
uint64 PluggableDeviceUtil::Checksum(Device* device,
                                     const DeviceContext* device_context,
                                     const Tensor& tensor) {
  Tensor copy(tensor.dtype(), tensor.shape());
  Status s;
  Notification n;
  CopyPluggableDeviceTensorToCPU(device, device_context, &tensor, &copy,
                                 [&s, &n](Status status) {
                                   s.Update(status);
                                   n.Notify();
                                 });
...
uint64 PluggableDeviceUtil::Checksum(const Tensor& tensor) {
