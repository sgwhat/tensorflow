...
int GPUProcessState::BusIdForGPU(TfDeviceId tf_gpu_id) {
  // Return the NUMA node associated with the GPU's StreamExecutor.
  se::StreamExecutor* se =
      DeviceIdUtil::ExecutorForTfDeviceId(GPUMachineManager(), tf_gpu_id)
          .ValueOrDie();
...
SharedCounter* GPUProcessState::GPUAllocatorCounter(TfDeviceId tf_gpu_id) {
...
Allocator* GPUProcessState::GetGpuHostAllocator(int numa_node) {
  ...
  for (int i = 0; i < static_cast<int>(gpu_allocators_.size()); ++i) {
    if (gpu_allocators_[i].allocator != nullptr) {
      se = DeviceIdUtil::ExecutorForTfDeviceId(GPUMachineManager(),
                                               TfDeviceId(i))
               .ValueOrDie();
      break;
    }
