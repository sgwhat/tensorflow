...
// must define.
typedef void (*TFInitGraphPluginFn)(TP_OptimizerRegistrationParams*,
                                    TF_Status*);
...
class CGraphOptimizer : public CustomGraphOptimizer {
 public:
  ...
  std::string name() const { return "PluginOptimizer"; }
...
#define CONFIG_BOOL(optimizer)                \
  if (tp_configs.optimizer == TF_TriState_On) \
    configs.optimizer = 1;                    \
  else                                        \
    configs.optimizer = 0;

class CGraphOptimizerRegistrar {
 public:
  explicit CGraphOptimizerRegistrar(
