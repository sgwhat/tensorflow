  ...
  def _initialize_physical_devices(self):
    ...
    with self._device_lock:
      ...
      self._physical_devices = [
            PhysicalDevice(name=d.decode().strip(d.decode().split(":")[3]).strip(":") if d.decode().count(":") == 3 else d.decode() ,
                         device_type=d.decode().split(":")[1],
                         subdevice_type=d.decode().split(":")[3] if d.decode().count(":") == 3 else d.decode().split(":")[1]) for d in devs]
